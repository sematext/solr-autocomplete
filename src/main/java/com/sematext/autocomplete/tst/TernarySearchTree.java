...
/**
 * Implementation of ternary search tree. A Ternary Search Tree is a data structure that behaves in a manner that is
 * very similar to a HashMap.
 */
public class TernarySearchTree {
    ...
    // convenience variable for matchPrefix and sortKeys methods
    ...
    /**
     * Sets default maximum number of values returned from matchPrefix, matchPrefixString, matchAlmost, and
     * matchAlmostString methods.
     * 
     * @param num
     *            The number of values that will be returned when calling the methods above. Set this to -1 to get an
     *            unlimited number of return values. Note that the methods mentioned above provide overloaded versions
     *            that allow you to specify the maximum number of return values, in which case this value is temporarily
     *            overridden.
     */
     ...
     * @return TSTNode The node object indexed by key. This object is an instance of an inner class named
    ...
    /**
     * Returns alphabetical list of all keys in the tree that begin with prefix. Only keys for nodes having non-null
     * data are included in the list.
     * 
     * @param prefix
     ...
     * @param prefix
     *            Each key returned from this method will begin with the characters in prefix.
     * @return String A string representation of all keys matching the argument prefix. Keys are delimited with the
     *         newline char ('\n').
     */
    ...
    private DoublyLinkedList sortKeysResult; // convenience variable for
    // matchPrefix and sortKeys
    // methods

    private boolean sortKeysList; // convenience variable for matchPrefix,
    // matchPrefixString and sortKeys methods

    private StringBuffer sortKeysBuffer; // convenience variable for
    // matchPrefixString and sortKeys
    // methods

    private int sortKeysNumReturnValues; // convenience variable for

    // matchPrefix and sortKeys methods

    /*
     * Returns keys sorted in alphabetical order. Includes currentNode and all nodes connected to currentNode. Sorted
     * keys will be appended to end of result list. (result may be empty when this method is invoked, but may not be
     * null.)
     */
    ...
    /**
     * Returns keys sorted in alphabetical order. Includes startNode and all nodes connected to startNode. Number of
     * keys returned is limited to numReturnValues. To get a list that isn't limited in size, set numReturnValues to -1.
     * 
     * @param startNode
     *            The top node defining the subtree to be searched.
     * @param numReturnValues
    ...
    /**
     * Returns keys sorted in alphabetical order, returning a result of type String. Includes startNode and all nodes
     * connected to startNode. Number of keys returned is limited to numReturnValues. To get a list that isn't limited
    ...
    private DoublyLinkedList matchAlmostResult; // convenience variable for
    // matchAlmost

    private StringBuffer matchAlmostBuffer; // convenience variable for
    // matchAlmostString

    private boolean matchAlmostListAction; // convenience variable for
    // matchAlmost and matchAlmostString

    private int matchAlmostNumReturnValues; // convenience variable for
    // matchAlmost
    ...
    /**
     * Returns a list of keys that almost match argument key. Keys returned will have exactly diff characters that do
     * not match the target key, where diff is equal to the last value passed in as an argument to the
     * setMatchAlmostDiff method. If the matchAlmost method is called before the setMatchAlmostDiff method has been
    ...
    /**
     * Returns a String representation of keys that almost match argument key. Keys returned will have exactly diff
     * characters that do not match the target key, where diff is equal to the last value passed in as an argument to
     * the setMatchAlmostDiff method. If the matchAlmost method is called before the setMatchAlmostDiff method has been
     * called for the first time, then diff = 0.
     * 
     * @param key
     *            The target key.
     * @return String A String representation of keys that almost match the target key. Keys are delimited by the
     *         newline char ('\n').
     */
    ...
    private void matchAlmostRecursion(TSTNode currentNode, int charIndex, int d) {
        if ((currentNode == null) || (d < 0) || (matchAlmostNumReturnValues == 0)
        ...
        /*
         * String str = (charComp < 0) ? "  Lo" : "  Hi"; System.out.println("" + charIndex + "   key: " +
         * matchAlmostKey.charAt(charIndex) + "   split:" + currentNode.splitchar + str);
         */
        ...
        if ((matchAlmostKey.length() == charIndex + 1) && (nextD == 0) && (currentNode.data != null)) {
            // Note: the condition nextD == 0 causes keys to be included in the
            // result only if they have exactly matchAlmostDiff number
            // of mismatched letters
            // If instead the condition nextD >= 0 is used, then all keys having
            // up to and including matchAlmostDiff mismatched letters
            // will be included in the result (including a key that is exactly
    ...
    /**
     * Sets the number of characters by which words can differ from target word when calling matchAlmost or
     * matchAlmostString methods. Arguments less than 1 will set the char difference to 1, and arguments greater than 4
    ...
    /**
     * Returns the Node indexed by key, creating that node if it doesn't exist, and creating any required. intermediate
    ...
    /*
     * Deletes the node passed in as an argument to this method. If this node has non-null data, then both the node and
     * the data will be deleted. Also deletes any other nodes in the tree that are no longer needed after the deletion
     * of the node first passed in as an argument to this method.
     */
    ...
    private TSTNode deleteNodeRecursion(TSTNode currentNode) {
        ...
        if (currentNode.relatives[TSTNode.EQKID] != null || currentNode.data != null)
            return null; // can't delete this node if it has a non-null eq
        // kid or data
        ...
        if (lokidNull && hikidNull) {
            // if we make it to here, all three kids are null and we can just
            // delete this node
            currentParent.relatives[childType] = null;
            return currentParent;
        }
        // if we make it this far, we know that EQKID is null, and either HIKID
        // or LOKID is null, or both HIKID and LOKID are NON-null
        if (lokidNull) {
        ...
        int movingKid;
        TSTNode targetNode;
        // if deltaHi is equal to deltaLo, then choose one of them at random,
        // and make it "further away" from the current node's splitchar
        if (deltaHi == deltaLo) {
            if (Math.random() < 0.5) {
                deltaHi++;
            } else {
        ...
        // now targetNode.relatives[movingKid] is null, and we can put the
        // moving kid into it.
        ...
        // note that the statements above ensure currentNode is completely
        // dereferenced, and so it will be garbage collected
    ...
    private StringBuffer getKeyBuffer = new StringBuffer(); // convenience
    ...
    /**
     * Returns the total number of nodes in the subtree below and including startingNode. Counts nodes whether or not
     * they have data.
     * 
     * @param startingNode
    ...
    /**
     * Returns the number of nodes in the subtree below and including startingNode. Counts only nodes that have non-null
     * data.
     * 
     * @param startingNode
     *            The top node of the subtree. The node that defines the subtree.
    ...
    private void printNodeRecursion(TSTNode currentNode) {
        ...
        if (currentNode.relatives[TSTNode.PARENT] == null) {
            ...
            System.out.println("parent key   |" + getKey(currentNode.relatives[TSTNode.PARENT])
                    + "|       parent data: " + currentNode.relatives[TSTNode.PARENT].data);
        }
        if (currentNode.relatives[TSTNode.LOKID] == null) {
        ...
        if (currentNode.relatives[TSTNode.EQKID] == null) {
            ...
            System.out.println("eqkid key   |" + getKey(currentNode.relatives[TSTNode.EQKID])
                    + "|       equal kid data: " + currentNode.relatives[TSTNode.EQKID].data);
        }
        if (currentNode.relatives[TSTNode.HIKID] == null) {
